...
export class DashboardPage implements OnInit {
  ...
  protected readonly charts = computed<IChartData[]>(() => {
    ...
    if (!dashboard || dashboard.sites.length === 0) {
      return [];
    }
    ...
    return [
      {
        chartId: 'site-health',
        chartType: 'bar',
        labels: dashboard.sites.map((site) => site.siteName),
        dataSets: [
          {
            label: 'Request p95',
            data: dashboard.sites.map((site) => site.requests24h.p95Ms),
            backgroundColors: dashboard.sites.map(() => '--cyan-500'),
          },
          {
            label: 'DB p95',
            data: dashboard.sites.map((site) => site.database24h.p95Ms),
            backgroundColors: dashboard.sites.map(() => '--orange-500'),
          },
        ],
      },
    ];
  ...
  protected statusClass(site: SiteHealthDto): string {
    return `health-pill health-pill--${site.status.toLowerCase()}`;
  }
